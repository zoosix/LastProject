...
public interface ReplyMapper {
	@Select("SELECT no,email,msg,TO_CHAR(regdate,'YYYY-MM-DD HH24:MI:SS') as dbday,group_tab,group_step,group_id,num "
			+ "FROM (SELECT no,email,name,msg,regdate,group_tab,group_step,group_id,rownum as num "
			+ "FROM (SELECT no,email,name,msg,regdate,group_tab,group_step,group_id FROM reply "
	...
	@Select("SELECT email,msg,TO_CHAR(regdate,'YYYY-MM-DD HH24:MI:SS') as dbday FROM reply WHERE no=(SELECT NVL(MAX(no),1) "
	...
	@Insert("INSERT INTO reply VALUES(#{no},#{bno},#{email},#{name},#{msg},SYSDATE,(SELECT NVL(MAX(group_id)+1,1) FROM reply),0,0,0,0)")
	...
	@SelectKey(keyProperty = "no", before = true, resultType = int.class, statement = "SELECT NVL(MAX(no)+1,1) as no FROM reply")
	@Insert("INSERT INTO reply VALUES(#{no},#{bno},#{email},#{name},#{msg},SYSDATE,#{group_id},#{group_step},#{group_tab},0,0)")
